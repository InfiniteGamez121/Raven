class Rhodium {
        ...
        {},
        {
          set(og, ob, val) {
            if (
              ...
              return true;
            return (location[ob] = URLParser.encode(
              $Rhodium.Url.href.replace($Rhodium.Url[ob], val),
              $Rhodium
            ));
          },
          get(og, ob) {
      ...
      return new Proxy(window.fetch, {
        apply(t, g, a) {
          if (a[0] && typeof a[0] == "object") {
            ...
            if ($Rhodium.replit)
              a[0].url = a[0].url.replace("https://", "https:/");
          } else if (a[0]) {
  ...
  Object.defineProperty(document, "cookie", {
    get() {
      ...
      cookie_array.forEach((cookie) => {
        ...
        if (
          $Rhodium.Url.hostname.includes(cookie_name.split("@").splice(1).join())
        )
          new_cookie.push(
  ...
  $Rhodium = new Rhodium(
    JSON.parse(document.currentScript.getAttribute("data-config"))
  );
    ...
    return {
      encode(url, context) {
        try {
          ...
          const rewritten =
            ctx.prefix + (validProtocol ? url : context.Url.origin + url);
          //throw new Error('');
          if (context.replit)
            ...
            apply(target, thisArg, [element_attribute, value]) {
              attribute_array.forEach((array_attribute) => {
                ...
                if (element_attribute.toLowerCase() == array_attribute)
                  value = URLParser.encode(value || "", rhodium);
              });
